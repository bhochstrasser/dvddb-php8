...
if (!saneempty($action)) {
	...
	switch($action) {
		case "edit":
			...
			if ($_GET["id"]!="0") {
				$result=mysqli_fetch_row(doquery("select ".$data["desc"]." from ".$data["name"]." where ".$data["key"]." = \"".greatescape($_GET["id"])."\""));
				$value=$result[0];
			...
			if ($data["name"]!="genre"&&$data["name"]!="media"&&$_GET["id"]=="0") {
		...
		break;
		case "delete":
			$result=mysqli_fetch_row(doquery("select count(*) from ".$data["usedin"]." where ".$data["fkey"]." = \"".greatescape($_GET["id"])."\""));
			if ($result[0]!=0) {
